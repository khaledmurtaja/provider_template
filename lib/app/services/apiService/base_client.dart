...
import 'package:provider_template/app/services/apiService/api_exceptions.dart';
...
class BaseClient {
  ...
  static _handleTimeoutException(
    ...
    if (onError != null) {
      onError(ApiException(
        message: S.current.serverNotResponding,
        url: url,
      ));
    } else {
      _handleError(S.current.serverNotResponding);
    }
  }
  ...
  static _handleSocketException(
    ...
    if (onError != null) {
      ...
      _handleError(S.current.noInternetConnection);
    }
  }
  ...
  static _handleDioError(
    ...
    if (error.response?.statusCode == 404) {
      if (onError != null) {
        return onError(ApiException(
          message: S.current.noUrlFound,
          url: url,
          statusCode: 404,
        ));
      } else {
        return _handleError(S.current.noUrlFound);
      }
    }
    ...
    if (error.message!.toLowerCase().contains('socket')) {
      if (onError != null) {
        ...
        return _handleError(S.current.noInternetConnection);
      }
    }
  ...
  static handleApiError(ApiException apiException) {
    ...
    print("$msg  حط ودجت بتدلل على الايرور");
